...
local function create_floating_file(location, opts)
  ...
  local contents = vim.api.nvim_buf_get_lines(
    ...
    range.start.line,
    math.min(
      range["end"].line + 1 + (opts.context or defaults.context),
      range.start.line + (opts.max_height or defaults.height)
    ),
    false
  )
  ...
  opts = vim.lsp.util.make_floating_popup_options(if_nil(width, defaults.width), if_nil(height, defaults.height), opts)
